...
// Build index
const bySection = new Map(); // section -> { sections: Set(ids), concepts: Map(id|alias|slug -> canonicalId) }
const idToSlug = new Map();

for (const f of files) {
  const rel = path.relative(DIR, f);
  const slug = path.basename(rel, ".md");
  const fm = matter.read(f).data || {};
  const id = fm.id || slug;
  const sectionId = fm.kind === "section" ? id : fm.category;
  ...
  if (fm.kind === "section") {
    ...
    for (const a of fm.aliases || []) bucket.concepts.set(a, id);
  }
}
...
for (const f of files) {
  ...
  const out = src.replace(LINK_RE, (m, pre, url, post) => {
    try {
