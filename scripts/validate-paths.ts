...
function collectGuidePathsWithSources() {
  ...
  for (const f of files) {
    ...
    const list = Array.isArray((fm.data as any).paths)
      ? ((fm.data as any).paths as string[])
      : [];
    for (const t of list) {
      const key = String(t);
...
function collectExistingPathSlugs() {
  if (!fs.existsSync(PATHS_DIR)) return new Set<string>();
  const files = fs
...
function buildGuideMap() {
  ...
  const map = new Map<
    string,
    { slug: string; paths: string[]; path: string }
  >();
  ...
  for (const f of files) {
    ...
    const paths = Array.isArray((fm.data as any).paths)
      ? ((fm.data as any).paths as string[])
      : [];
