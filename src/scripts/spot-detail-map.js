...
import { createBaseMap, waitForLeaflet, createMarkerWithPopup } from '../utils/leaflet-setup';
...
function initSpotMap() {
  waitForLeaflet(() => {
    ...
    const map = createBaseMap('spot-map', [spot.latitude, spot.longitude], MAP_DEFAULTS.SPOT_ZOOM);
    if (!map) return;
